...
class SignUpUseCase @Inject constructor(private val authRepository: AuthRepositoryImpl) {
    ...
    operator fun invoke(user: User, verifyCode: String): Flow<Result<Task<SignInResult>>> = flow {
        try {
            emit(Result.Success(data = authRepository.signUp(user = user, verifyCode = verifyCode)))
        } catch (e: IOException) {
